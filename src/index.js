...
// Connect to database
mongoose.connect(config.MongoDBURl, { useNewUrlParser: true, useUnifiedTopology: true }).then(() => {
...
})).use(compression()).use(bodyParser.urlencoded({ extended: true })).use(bodyParser.json()).use(session({
	store: new mStore({ checkPeriod: 86400000 }),
	secret: 'secret',
	resave: false,
	saveUninitialized: false,
})).use(passport.initialize()).use(passport.session()).use(function (req, res, next) {
	if (req.originalUrl !== '/favicon.ico') logger.connection(req, res);
	next();
}).use(flash()).engine('html', require('ejs').renderFile).set('view engine', 'ejs').set('views', './src/website/views').use(express.static('./src/website/public')).use(favicon('./src/website/assets/favicon.ico')).use(require('./website/config/RateLimit').connectionLimiter).use('/', require('./website/router')).use('/files', require('./website/router/files')).use('/user', require('./website/router/user')).use('/auth', require('./website/router/auth')).use('/social', require('./website/router/social')).use('/api', require('./website/router/api')).use('/admin', require('./website/router/admin')).use(function (error, req, res, next) {
...
if (config.secure) {
	try {
		...
		}, app).listen(6500, () => logger.log('HTTPS server online (port: 443)', 'ready'));
	} catch (err) {
		console.log(`HTTPS server not online due to ${err.message}`);
	}
} else {
	// Create an HTTP service.
	http.createServer(app).listen(6500, () => logger.log('HTTP server online (port: 80)', 'ready'));
}
